...
import {
  ...
  ChevronRight, Loader2, Check, Plus
} from 'lucide-react';
...
export default function HomePage() {
      ...
      try {
        ...
        if (user) {
          ...
          if (listRes.data.success) {
            setMyListIds(new Set(listRes.data.data.map((s: any) => s._id)));
          }
        }
      } catch (err) {
  ...
  const handleToggleMyList = async (seriesId: string) => {
    if (!user) return alert('Please login to add to your list');

    try {
      setTogglingId(seriesId);
      const isCurrentlyInList = myListIds.has(seriesId);

      if (isCurrentlyInList) {
        await api.delete(`/users/mylist/${seriesId}`);
        setMyListIds(prev => {
        ...
        await api.post(`/users/mylist/${seriesId}`);
        setMyListIds(prev => {
          const next = new Set(prev);
          next.add(seriesId);
          return next;
            ...
            <div className="relative z-20 h-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col justify-center pb-20 pt-10">
              <div className="max-w-3xl space-y-6 animate-fade-in-up">
                ...
                <div className="flex items-center gap-4 pt-4">
                  ...
                  <button
                    onClick={() => handleToggleMyList(item._id)}
                    disabled={togglingId === item._id}
                    className={clsx(
                      "size-14 rounded-2xl flex items-center justify-center border transition-all backdrop-blur-md hover:scale-105",
                      myListIds.has(item._id)
